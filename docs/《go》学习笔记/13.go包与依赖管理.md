...
![包初始化函数执行顺序示意图](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/202202161330443.png)
...
这种方法同样支持指定想要下载的commit进行下载，例如直接在`go.mode`文件中按如下方式指定commit hash，这里只写出来了commit hash的前7位。
...
![go modules生成的版本信息组成示意图](https://www.liwenzhou.com/images/Go/package/module_version_info.png)

**go.sum文件**

使用go modules下载了依赖后，项目目录下还会生成一个`go.sum`文件，这个文件中详细记录了当前项目中引入的依赖包的信息及其hash 值。`go.sum`文件内容通常是以类似下面的格式出现。
...
func main() {
	sumret := calc.Sum(20, 10)
	fmt.Println(sumret)
	subret := calc.Sub(20, 10)
	fmt.Println(subret)
	mulret := calc.Mul(20, 10)
	fmt.Println(mulret)
	divret := calc.Div(20, 10)
	fmt.Println(divret)
