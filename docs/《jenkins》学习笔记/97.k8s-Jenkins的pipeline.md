---
title: k8s-Jenkins的pipeline
date: 2020-09-08 10:13:16
permalink: /pages/b5e142139/
categories:
  - 《jenkins》学习笔记
tags:
  - jenkins
...
k8s-Jenkins的pipeline

前置环境 

[jenkins-k8s](/pages/b5e14200/)

[gitlab-k8s](/pages/b5e14521999/)
...
- 新建任务：在 Web UI 中点击 `新建任务` -> 输入名称：pipeline-demo -> 选择下面的 `流水线` -> 点击 `确定`
  ...
  ![image-20211010205424481](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010205424481.png)
...
- scr构建：点击左侧区域的 `立即构建`，可以看到 Job 开始构建了

  ![image-20211010205824857](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010205824857.png)

隔一会儿，构建完成，可以点击左侧区域的 `Console Output`，我们就可以看到如下输出信息：

![image-20211010205548140](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010205548140.png)
...
kube-ops      jenkins-d59b57f9d-dpsmp                      1/1     Running   0          5h42m
```
...
$ kubectl get pods -n kube-ops
NAME                                           READY   STATUS    RESTARTS   AGE
kube-ops      jenkins-agent-1sbsr                          1/1     Running   0          10s
kube-ops      jenkins-d59b57f9d-dpsmp                      1/1     Running   0          5h42m
```

我们发现多了一个名叫 `jenkins-d59b57f9d-dpsmp  ` 的 Pod 正在运行，隔一会儿这个 Pod 就不再了。这也证明我们的 Job 构建完成了，同样回到 Jenkins 的 Web UI 界面中查看 Console Output，可以看到如下的信息：

![image-20211010210754428](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010210754428.png)

`pipeline demo#2` 是不是也证明我们当前的任务在跑在上面动态生成的这个 Pod 中，也符合我们的预期。
...
![jenkins pipeline demo](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20210601150311.png)
...
![gitlab 插件](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20210601172935.png)
...
![image-20211010224609316](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010224609316.png)
...
>
> 7f27389c4241dbe06f53f57168810ebb http://jenkins.k8s.local:26052/project/devops-demo



```
http://jenkins.k8s.local:26052/project/devops-demo?token=jenkins123
```

![image-20211010231625742](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010231625742.png)
...
![](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010232657820.png)
...
![image-20211010232657820](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010232657820.png)

在 `Credentials` 区域点击添加按钮添加我们访问 Gitlab 的用户名和密码：

![image-20211010232917244](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211010232917244.png)
...
最后点击保存，至此，Jenkins 的持续集成配置好了

### 3.2、gilab 的Webhook关联Jenkins

还需要配置 Gitlab 的 Webhook，用于代码提交通知 Jenkins。前往 Gitlab 中配置项目 devops-demo 的 Webhook，settings -> Webhooks，填写上面得到的 trigger 地址：

![image-20211011195004941](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211011195004941.png)

![image-20211011200304942](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211011200304942.png)
...
![image-20211011200826513](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211011200826513.png)
...
![pipeline workflow](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20210603193152.png)
...
```groovy
Groovy def label = "slave-${UUID.randomUUID().toString()}"
...
![image-20211012224925029](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/image-20211012224925029.png)
...
![harbor project](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20200520142740.png)

Docker 的用户名和密码信息则需要通过凭据来进行添加，进入 jenkins 首页 -> 左侧菜单凭据 -> 添加凭据，选择用户名和密码类型的，其中 ID 一定要和上面的 `credentialsId` 的值保持一致：

![harbor auth](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20200520140737.png)
...
![docker 命令](https://bxdc-static.oss-cn-beijing.aliyuncs.com/images/20210604203124.png)
...
```sh
 kubectl create secret docker-registry harbor-auth --docker-server=my.harbor.com --docker-username=admin --docker-password=123 --docker-email=admin@admin.com --namespace kube-ops
```
...
![img](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20200520161259.png)
...
![img](https://cdn.jsdelivr.net/gh/lzq70112/images/blog/20200520163020.png)
